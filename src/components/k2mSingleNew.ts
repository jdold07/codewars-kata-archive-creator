...
import existingCompleted from "../../private/assets/completedKata"
...
// Fetch list of completed Katas to obtain completion details
async function getCompletedKatasJSON() {
  try {
    ...
    completedKata.forEach((kata: any) => {
      ...
      fs.writeFileSync(join("./private/assets/completedKata.json"), JSON.stringify(existingCompleted), {
        flag: "w",
        encoding: "utf8",
        mode: 644
      })
...
async function mergeData(kata: any, lang: string) {
  const solutions = await scrapeHTML()
